...
export default function Home() {
  ...
  const loadData = async () => {
    try {
      ...
      // 2. Fetch timeseries and anomalies for each metric concurrently
      const overviews: MetricOverviewData[] = await Promise.all(
        metrics.map(async (m) => {
          try {
            const [{ points }, anomalies] = await Promise.all([
              fetchTimeseries(m.id),
              fetchAnomalies(m.id),
            ]);
            ...
            if (points.length > 0) {
              ...
              // Find new anomalies in the last 7 days of dataset
  ...
  // Error State: Enterprise Diagnostics Experience
  if (error) {
    return (
      ...
      <div className="mx-auto max-w-7xl relative">
        ...
        >
          {metricOverviews.map(({ metric, latestValue, points, recentAnomaly, pctChange, absChange }) => {
            ...
            // 30 days sparkline SVG calculation with SVG gradient fill
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="flex items-end justify-between gap-4 mt-6 mb-6">
                      ...
                      {/* Render sparkline SVG with Area Gradient */}
                      {sparklineSvgPath && (
