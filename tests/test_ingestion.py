...
async def test_csv_validation_report_catches_errors_and_gaps():
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
        metric_res = await client.post("/metrics", json={"name": "Validation Test Metric"})
        assert metric_res.status_code == 201
        metric_id = metric_res.json()["id"]
...
"""
        ...
        upload_res = await client.post(f"/metrics/{metric_id}/data", files={"file": ("test_err.csv", file_obj, "text/csv")})
        assert upload_res.status_code == 200
...
async def test_append_vs_replace_semantics():
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
        metric_res = await client.post("/metrics", json={"name": "Append Test Metric"})
        assert metric_res.status_code == 201
        metric_id = metric_res.json()["id"]
...
"""
        upload_res1 = await client.post(f"/metrics/{metric_id}/data", files={"file": ("init.csv", io.BytesIO(initial_csv.encode("utf-8")), "text/csv")})
        data1 = upload_res1.json()

        confirm_res1 = await client.post(f"/metrics/{metric_id}/data/confirm", json={
            "date_col": "date",
            "value_col": "revenue",
...
"""
        upload_res2 = await client.post(f"/metrics/{metric_id}/data", files={"file": ("add.csv", io.BytesIO(additional_csv.encode("utf-8")), "text/csv")})
