...
async def test_all_tables_exist_and_valid():
    EXPECTED_TABLES = {
        ...
        "daily_rollups": ["id", "metric_id", "date", "value_total", "trend", "seasonal", "residual", "dimension_values"],
    ...
    async with test_engine.connect() as conn:
        def check_schema(sync_conn):
            ...
            # Assert unique index on daily_rollups uq_daily_rollups_metric_date_dims
            rollup_indexes = [idx["name"] for idx in inspector.get_indexes("daily_rollups")]
